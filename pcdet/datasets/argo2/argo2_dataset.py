...
def process_and_save_frame(frame_path, frame_anno, ts2idx, segname, output_dir, save_bin):
    ...
    frame_info['annos'] = dict(
        ...
        gt_boxes_lidar=None, # gorkem added this part because we need this key!!!
    )
    ...
    if frame_anno is not None:
        ...
        annos['gt_boxes_lidar']= gt_bboxes_3d
...
class Argo2Dataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.update_annos() # GORKEM ADDED THIS - TEMP SOLUTION - if you already generated the pickles and avoid redo. this adds 3d bbox key into annos.


    def update_annos(self,):
        for c_frame in self.infos: 
            loc, dims, rots = c_frame['annos']['location'], c_frame['annos']['dimensions'], c_frame['annos']['rotation_y']
            gt_names = c_frame['annos']['name']
            gt_bboxes_3d = np.concatenate([loc, dims, rots[..., np.newaxis]], axis=1).astype(np.float32)
            c_frame['annos']['gt_boxes_lidar'] = gt_bboxes_3d
    ...
    def format_results(self,
        ...
        assert len(self.infos) == len(outputs)
        ...
        for i in range(num_samples):
            out_i = outputs[i]
            log_id, ts = self.infos[i]['uuid'].split('/')
            track_uuid = None
