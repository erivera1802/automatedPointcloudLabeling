...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        #split_dir = '/MS3D/data/kitti/ImageSets' / (self.split + '.txt')
        #split_dir = Path("/MS3D/data/kitti/ImageSets/train.txt")
        #print("split dire nedir", split_dir)
        self.sample_id_list = [x.strip() for x in open(split_dir).readlines()] if split_dir.exists() else None
        
        print("HEYYYY",len(self.sample_id_list))
    ...
    def include_kitti_data(self, mode):
        ...
        if self.use_raw_data:
            ...
            print("INFO_PATH_ mode", self.dataset_cfg.INFO_PATH[mode], )
    ...
    def get_raw_data_infos(self):
        ...
        def get_tracklets_for_sample_idx(sample_idx, tracklets):        
            ...
            for track in tracklets:
                ...
                # if track.objectType == 'DontCare': #EDITED - MAYBE WE CAN USE LABELS TXT HERE TO CORRECT RESULTS? 
                #     print(f'BOX DATA {track.boxes[box_idx.item()]} AND NAME {len(names)} LENGTHS ARE DIFFERENT')
                #     break
            # if len(frame_boxes) != len(names): #EDITED - MAYBE WE CAN USE LABELS TXT HERE TO CORRECT RESULTS? 
            #     print(f'BOX {len(frame_boxes)} AND NAME {len(names)} LENGTHS ARE DIFFERENT')
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                # if len(annotations['gt_boxes_lidar']) != len(annotations['name']): #EDITED - MAYBE WE CAN USE LABELS TXT HERE TO CORRECT RESULTS? 
                #     print('what',len(annotations['gt_boxes_lidar']) , len(annotations['name']))
...
if __name__ == '__main__':
    ...
    # python -m pcdet.datasets.kitti.kitti_dataset create_kitti_infos tools/cfgs/dataset_configs/kitti_raw_dataset_da.yaml
