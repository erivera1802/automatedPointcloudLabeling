...
def get_detection_sets(det_annos, score_th=0.1):
    ...
    for idx in tqdm(range(len_data), total=len_data, desc='load detection sets'):
        ...
        for src_id, key in enumerate(src_keys):
            ...
            # print("hey",pred_names) ####delete
            superclass_ids = np.array([SUPERCATEGORIES.index(SUPER_MAPPING[name]) for name in pred_names]) # (SUPER_MAPPING[name])+1 !!!!
...
def get_initial_pseudo_labels(detection_sets, cls_kbf_config): 
    ...
    for frame_boxes in tqdm(detection_sets, total=len(detection_sets), desc='get initial ps labels'):
        ...
        ps_label_nms = []
        for class_name in SUPERCATEGORIES: #fusing class by class
            cls_mask = (frame_boxes['names'] == class_name) ### FutureWarning: elementwise comparison failed; returning scalar instead, but in the future will perform elementwise comparison
        ...
        # Currently we only store pseudo label information for each frame ##### UPDATE HERE TO GET MORE KEYS
        gt_infos = {
            'bbox': np.zeros((ps_label_nms.shape[0],4)),
            'gt_boxes': gt_box,
            # 'dimension': pred_boxes[:,:3],
            # 'location': pred_boxes[:,3:6],
            # 'rotation': pred_boxes[:,6],
            # 'score': pred_scores,
            # 'alpha': np.zeros(ps_label_nms.shape[0]),
            # 'name': frame_boxes['names'],
        } 
        ps_dict[frame_boxes['frame_id']] = gt_infos
        # ps_array.append(gt_infos)
