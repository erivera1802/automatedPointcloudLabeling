...
python3 visualize_bev.py --cfg_file cfgs/dataset_configs/kitti_raw_dataset_da.yaml \
                        --ps_pkl cfgs/target_kitti/label_generation/round1/ps_labels/used_waymo_lyft.pkl \
                        --idx 100 

python3 visualize_bev.py --cfg_file cfgs/dataset_configs/custom_dataset_da.yaml \
                        --ps_pkl cfgs/target_custom/label_generation/round1_automated_pipeline/ps_labels/2024_04_23_bakery_sensorsplussensing_000.pkl \
...
def main():
    ...
    parser.add_argument('--hide_score', action='store_true', default=True)
    ...
    class_mask = np.isin(compat.get_gt_names(target_set, start_frame_id), ['Vehicle','car','Car','truck','bus'])[:shorten_index]
    # print(class_mask, shorten_index)
    plot_boxes(ax, compat.get_gt_boxes(target_set, start_frame_id)[class_mask], color=[0,0,1],
        limit_range=limit_range, label='gt_vehicle', linewidth=5,
    ...
    plot_boxes(ax, compat.get_gt_boxes(target_set, start_frame_id)[class_mask], color=[0.5,0,0.5],
        limit_range=limit_range, label='gt_pedestrian', linewidth=3,
        scores=np.ones(compat.get_gt_boxes(target_set, start_frame_id)[class_mask].shape[0]), show_score=False if args.hide_score else False)
    
    class_mask = np.isin(compat.get_gt_names(target_set, start_frame_id), ['Cyclist','bicycle','motorcycle'])[:shorten_index]
    plot_boxes(ax, compat.get_gt_boxes(target_set, start_frame_id)[class_mask], color=[0,0.5,0.5], linewidth=3,
    ...
    if args.ps_pkl is not None:
        ...
        plot_boxes(ax, ps_dict[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask], 
                ...
                label='ps labels', color=[0,0.8,0], fontsize=12, linewidth=2,
                limit_range=limit_range, alpha=1, show_score=False if args.hide_score else True)
    ...
    def visualize(ind):
        ...
        if args.ps_pkl is not None:
            ...
            plot_boxes(ax, ps_dict[ps_frame_ids[frame_idx]]['gt_boxes'][combined_mask], 
                ...
                label='ps labels', color=[0,0.8,0], fontsize=14, linewidth=2,
                limit_range=limit_range, alpha=1, show_score=False if args.hide_score else True)             
    ...
    ax.set_aspect('equal')
    save_fig = f'/MS3D/autolabel_2024_04_23_bakery_sensorsplussensing_000/figure_{start_frame_id}.png'
    plt.savefig(save_fig)
    print('saved!!', save_fig)
