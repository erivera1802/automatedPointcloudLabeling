...
python visualize_bev.py --cfg_file cfgs/dataset_configs/waymo_dataset_da.yaml \
                        --dets_txt /MS3D/tools/cfgs/target_nuscenes/raw_dets/det_1f_paths.txt

python visualize_bev.py --cfg_file cfgs/dataset_configs/waymo_dataset_da.yaml \
                        --ps_pkl cfgs/target_waymo/pretrained/ps_label_e0.pkl

python3 visualize_bev.py --cfg_file cfgs/dataset_configs/custom_dataset_da.yaml \
                        --ps_pkl cfgs/target_custom/label_generation/round1/ps_labels/initial_pseudo_labels.pkl
"""
...
def main():
    ...
    # infos = {}
    # for i in target_set.argo2_infos:
    #     infos[i['sample_idx']] = i 

    # target_set.infos = infos

    # frameid_to_idx = {k: idx for idx,k in enumerate(infos.keys())} ### 100000 -> 0
    # target_set.frameid_to_idx = frameid_to_idx
    ...
    # a = target_set.infos[start_frame_id]['annos']['location']
    # b = target_set.infos[start_frame_id]['annos']['dimensions']
    # c = target_set.infos[start_frame_id]['annos']['rotation_y']
    ...
    #     temp.extend(y)
    #     temp.extend([z])
    #     gt_boxes_lidar.append(temp)
        
    # gt_boxes_lidar = np.array(gt_boxes_lidar)

    # target_set.infos[start_frame_id]['annos']['gt_boxes_lidar'] = gt_boxes_lidar


    pts = target_set[target_set.frameid_to_idx[start_frame_id]]['points'] # target_set[target_set.frameid_to_idx[start_frame_id]]['gt_boxes']
    pcr = 80
    ...
    if args.color_height: # is very slow - use only for paper vis. Buggy - doesn't do Next/Prev well
        ...
        scatter = ax.scatter(pts[:,0],pts[:,1],s=0.1, c='black', marker='o')

    # Plot GT boxes
    dont_care_mask = np.isin(compat.get_gt_names(target_set, start_frame_id), ['DontCare'])
    index_of_first_false = np.where(~dont_care_mask)[0][0] if False in dont_care_mask else -1
    index_of_first_true = np.where(dont_care_mask)[0][0] if True in dont_care_mask else -1
    shorten_index = len(dont_care_mask) #in case there is no dontcare
    if index_of_first_false != -1 and index_of_first_true != -1 and index_of_first_true > index_of_first_false:
        #valid for shortening, includes DontCare objects
        shorten_index = min(len(dont_care_mask),index_of_first_true)

    # print(compat.get_gt_names(target_set, start_frame_id))
    class_mask = np.isin(compat.get_gt_names(target_set, start_frame_id), ['Regular_vehicle','Vehicle','car','Car','truck','bus'])[:shorten_index]
    # print(class_mask, shorten_index)
    plot_boxes(ax, compat.get_gt_boxes(target_set, start_frame_id)[class_mask], color=[0,0,1],
        limit_range=limit_range, label='gt_vehicle', linewidth=3,
        scores=np.ones(compat.get_gt_boxes(target_set, start_frame_id)[class_mask].shape[0]), show_score=False if args.hide_score else False)
    
    class_mask = np.isin(compat.get_gt_names(target_set, start_frame_id), ['Pedestrian','pedestrian'])[:shorten_index]
    plot_boxes(ax, compat.get_gt_boxes(target_set, start_frame_id)[class_mask], color=[0.5,0,0.5],
        limit_range=limit_range, label='gt_pedestrian', linewidth=5,
        scores=np.ones(compat.get_gt_boxes(target_set, start_frame_id)[class_mask].shape[0]), show_score=False if args.hide_score else False)
    
    class_mask = np.isin(compat.get_gt_names(target_set, start_frame_id), ['Cyclist','Bicycle', 'Bicyclist', 'Wheelchair','motorcycle'])[:shorten_index]
    plot_boxes(ax, compat.get_gt_boxes(target_set, start_frame_id)[class_mask], color=[0,0.5,0.5],
        limit_range=limit_range, label='gt_cyclist', linewidth=5,
        scores=np.ones(compat.get_gt_boxes(target_set, start_frame_id)[class_mask].shape[0]), show_score=False if args.hide_score else False)
    ...
    if args.ps_pkl is not None:
        ...
        plot_boxes(ax, ps_dict[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask], 
                ...
                label='ps labels', color=[0,0.8,0], fontsize=12, linewidth=1,
                limit_range=limit_range, alpha=1, show_score=False if args.hide_score else True)
    ...
    def visualize(ind):
        ...
        plot_boxes(ax, compat.get_gt_boxes(target_set, frame_id)[class_mask], color=[0,0.5,0.5],
            limit_range=limit_range, label='gt_cyclist', linewidth=2,
            scores=np.ones(compat.get_gt_boxes(target_set, frame_id)[class_mask].shape[0]), show_score=False if args.hide_score else True)
        ...
        # save_fig = f'../data_temp/autolabel_argo_images/figure_{frame_idx}.png'
    ...
    ax.set_aspect('equal')
    save_fig = f'/MS3D/autolabel_argo_images/only_nuscenes_detections/figure_{start_frame_id}.png'
    plt.savefig(save_fig)
    print('saved!!', save_fig)
    plt.show()
